...
int rop_set_string(redisContext *conn, const char *key, const char *value)
{
	...
	if(reply == NULL)
	{
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "set :%s ,value = %s , error = %s", key, value, conn->errstr);
		ret = -1;
		goto END;
	}
...
int rop_get_string(redisContext *conn, const char *key, char *value)
{
	...
	if(reply == NULL)
	{
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "GET :%s error = %s", key, conn->errstr);
		ret = -1;
		goto END;
	}
 ...
 */
/* -------------------------------------------*/
int rop_hset_string(redisContext *conn, char *key, RFIELDS fields, RVALUES values)
{
	int ret = 0;
	redisReply *reply = NULL;
	reply = redisCommand(conn, "HSET %s %s %s", key, fields, values);
	if(reply == NULL)
	{
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "HSET :%s error = %s", key, conn->errstr);
		ret = -1;
		goto END;
	}
...
int rop_hget_string(redisContext *conn, char *key, RFIELDS fields, RVALUES values)
{
	int ret = 0;
	redisReply *reply = NULL;
	reply = redisCommand(conn, "HGET %s %s", key, fields);
	if(reply == NULL)
	{
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "HGET :%s error = %s", key, conn->errstr);
		ret = -1;
		goto END;
	}
